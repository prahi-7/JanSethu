  ...
  if (!problem) {
    return (
      <div className="flex min-h-screen bg-gradient-to-br from-[#FFF5F2] via-white to-blue-50 pt-16">
        ...
        <div className="flex-1 p-8 ml-64 flex items-center justify-center">
          <div className="text-center">
            <p className="text-gray-500 text-base md:text-lg">Problem not found</p>
            <Link to="/citizen/problems" className="text-[#FFCABE] hover:text-pink-700 mt-2 inline-block">
              ← Back to My Problems
            </Link>
          </div>
        </div>
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-[#FFF5F2] via-white to-blue-50 pt-16">
      ...
      <div className="flex-1 p-4 md:p-8 ml-0 md:ml-64">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6">
          <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-700">{problem?.title || 'Problem Details'}</h1>
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <span className="px-3 py-1 bg-[#FFF5F2] text-[#D4A09A] rounded-full text-xs border border-[#FFCABE]">Category</span>
            <span className="px-3 py-1 bg-red-50 text-red-500 rounded-full text-xs border border-red-200">High Priority</span>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center gap-2 text-gray-500">
              <FaMapMarkerAlt className="text-pink-400" /> Location
            </div>
            <div className="flex items-center gap-2 text-gray-500">
              <FaCalendar className="text-pink-400" /> Date
            </div>
            <div className="flex items-center gap-2 text-gray-500">
              <FaUser className="text-pink-400" /> Assigned to: Team
            </div>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-100">
            <h2 className="font-semibold text-gray-700 mb-3">Provide Feedback</h2>
            <div className="flex gap-1 mb-3">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <FaStar className={(hoveredRating || rating) >= star ? 'text-pink-400' : 'text-gray-300'} />
                </button>
              ))}
            </div>
            <textarea
            ...
            <button
              ...
              className="mt-2 px-4 py-2 bg-gradient-to-r from-[#FFCABE] to-[#E8B5A9] text-white rounded-xl font-semibold hover:shadow-lg transition-all text-sm"
            >
              Submit Feedback 💝
            </button>
          </div>
