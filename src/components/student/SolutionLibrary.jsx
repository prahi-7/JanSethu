...
import { FaSearch, FaStar, FaDownload, FaEye, FaBook } from 'react-icons/fa';
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-[#FFF5F2] via-white to-blue-50 pt-16">
      ...
      <div className="flex-1 p-4 md:p-8 ml-0 md:ml-64">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-700 mb-2">Solution Library</h1>
        <p className="text-sm md:text-base text-gray-400 mb-4 md:mb-6">Learn from successful solutions</p>
        ...
        {solutions.length === 0 ? (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-8 md:p-12 text-center">
            <FaBook className="text-4xl md:text-5xl text-gray-300 mx-auto mb-3 md:mb-4" />
            <p className="text-base md:text-lg text-gray-500">No solutions available</p>
            <p className="text-sm text-gray-400 mt-1">Solutions will appear once problems are solved</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {solutions.map((solution) => (
              <div key={solution.id} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 hover:shadow-md transition-all">
                {/* Solution details */}
              </div>
            ))}
          </div>
