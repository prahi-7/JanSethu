...
// Create a new problem (with Cloudinary file upload support)
  ...
  try {
    ...
    if (typeof location === 'string') {
      try {
        ...
        parsedLocation = {
          lat: parseFloat(
            req.body['location[lat]'] || req.body.lat || 0
          ),
          lng: parseFloat(
            req.body['location[lng]'] || req.body.lng || 0
          ),
          address:
    ...
    // Get existing photo/video URLs from body
    let existingPhotos = [];
    let existingVideos = [];
...
const deleteProblem = async (req, res) => {
  try {
    ...
    await ProblemTimeline.deleteMany({ problem: id });
...
const getCitizenProblems = async (req, res) => {
  try {
    ...
    const isOwnProblems =
      String(req.userId) === String(citizenId);

    const isAdminOrGovernment =
      req.userRole === 'admin' ||
      req.userRole === 'government';

    if (!isOwnProblems && !isAdminOrGovernment) {
    ...
    const [problems, total] = await Promise.all([
      Problem.find({ citizen: citizenId })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum),
...
const getMapProblems = async (req, res) => {
  try {
    ...
    logger.error('Get map problems error:', error);
...
const getProblemTimeline = async (req, res) => {
  try {
    ...
    const timeline = await ProblemTimeline.find({
      problem: id
    })
      .populate(
        'changedBy',
...
const assignProblem = async (req, res) => {
  try {
    ...
    if (department) {
      problem.assignedDepartment = department;
    }
    ...
    await ProblemTimeline.create({
      ...
      changedBy: req.userId,
      comment: comment || 'Problem assigned',
      assignmentInfo: {
    ...
    logger.error('Assign problem error:', error);
