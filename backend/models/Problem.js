...
const { PROBLEM_CATEGORIES, PROBLEM_STATUSES, PROBLEM_PRIORITIES } = require('../utils/constants');
...
problemSchema.methods.updateStatus = async function(newStatus, userId, comment = '') {
  this.status = newStatus;
  await this.save();
  
  const Timeline = mongoose.model('ProblemTimeline');
...
problemSchema.methods.assignTo = async function(userId, department, comment = '') {
...
problemSchema.statics.getCategoryDistribution = async function() {
...
// Static method: Get priority distribution
problemSchema.statics.getPriorityDistribution = async function() {
  return await this.aggregate([
    {
      $group: {
        _id: '$priority',
